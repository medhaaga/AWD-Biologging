...
from src.utils.conformal import validate, get_logits_targets, sort_sum

"""This code is adapted from https://github.com/aangelopoulos/conformal_classification
"""
...
class ConformalModel(nn.Module):
    def __init__(self, model, calib_loader, alpha, kreg=None, lamda=None, randomized=True, allow_zero_sets=False, pct_paramtune = 0.3, batch_size=32, lamda_criterion='size'):
        ...
        self.num_classes = len(torch.unique(calib_loader.dataset[:][1]))
...
def conformal_calibration(cmodel, calib_loader):
    ...
    with torch.no_grad():
        ...
        for x, targets in tqdm(calib_loader):
            x = x.to(cmodel.device)
            logits = cmodel.model(x).detach().cpu().numpy()
...
def platt_logits(cmodel, calib_loader, max_iters=10, lr=0.01, epsilon=0.01):
    nll_criterion = nn.CrossEntropyLoss().to(cmodel.device)

    T = nn.Parameter(torch.Tensor([1.3]).to(cmodel.device))
    ...
    for iter in range(max_iters):
        ...
        for x, targets in calib_loader:
            ...
            out = x/T
            loss = nll_criterion(out, targets.long().to(cmodel.device))
            loss.backward()
            optimizer.step()
